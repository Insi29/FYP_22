...
from src.utils.word_recognition import word_recog_predict
    ...
    def run(self):
        ...
        while cap.isOpened():
                    ...
                    if len(blackboard_cnts) >= 1:
                        ...
                        for cnt in blackboard_cnts:
                            if cv2.contourArea(cnt) > 1000:
                                 ...
                                 #print('Calculating bouding rct, pixels')
                                 #char = blackboard_gray[y:y + h, x:x + w]
                                 #Ocr Modularized, takes img as input and generates predicted text
                                 #txt_generated=ocr(char)
                                 #res_list.append(txt_generated)
                                 frame_count = 0
            ...
            if k & 0xFF == 27: #esc pressed which closes the screen
                ...
                #SPACE pressed
                img_name = "C:/Users/dsdfhj/Downloads/FYP_MIDYEAR/opencv_frame_{}.png".format(img_counter)
                cv2.imwrite(img_name, blackboard)
                print("{} written!".format(img_name))
                img_counter += 1
                print('File path:',img_name)
                #prediction of text
                res=word_recog_predict(img_name)
                print('Text generatd',res)
                
                image = cv2.putText(image, "Text =  ", (600, 50),
                                    cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 3)
            image = cv2.putText(image, "FYP", (920, 50),
                                    cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 3)
